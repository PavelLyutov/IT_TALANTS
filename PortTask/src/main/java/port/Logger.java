...
//            getFattestShip();
    ...
    private void listAllPackagesPerDock() {
        //1 - get all packages from db
        //2 - iterate them and place them into a bucket corresponding to their dock id
        // dock_id -> list of packages sorted by date

        TreeMap<Integer, TreeSet<Shipment>> stats = new TreeMap<>();
        String sql = "SELECT boat_name, dock_id, crane_id, unloading_time, package_id FROM port_shipments";
    ...
    private void saveToFile(TreeMap<Integer, TreeSet<Shipment>> stats) {
        try(PrintWriter pr = new PrintWriter("report-"+stats1Counter++ + "-" + LocalDateTime.now().toString().replace(":","-").replace("\\.","-") + ".txt");) {
            for(Map.Entry<Integer, TreeSet<Shipment>> e : stats.entrySet()){
